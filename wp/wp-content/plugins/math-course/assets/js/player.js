document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateProgressUI(progress, lessonId) {
        ...
        if (lessonId) document.querySelectorAll('.mc-course-player__item').forEach(function (item) { if ((item.getAttribute('href') || '').indexOf('lesson_id=' + lessonId) !== -1) { item.classList.add('is-complete'); const check = item.querySelector('.mc-course-player__check'); if (check) check.textContent = '✓'; } });
    }
    players.forEach(function (element) {
        ...
        let player = element, completionSent = false, restorePending = true, lastSavedSecond = -1, seeking = false, lastTime = 0, lastWallClock = 0, playedSinceSeek = 0, completedAt = 0;
        ...
        function on(target, event, callback) { if (target && typeof target.on === 'function') target.on(event, callback); else if (target && target.addEventListener) target.addEventListener(event, callback); }
        function setTime(time) { if (typeof player.currentTime === 'function') player.currentTime(time); else player.currentTime = time; }
        function getSavedTime() { try { const raw = localStorage.getItem(storageKey), time = raw === null ? 0 : parseFloat(raw); return Number.isFinite(time) && time > 0 ? time : 0; } catch (e) { return 0; } }
        function clearSavedTime() { try { localStorage.removeItem(storageKey); } catch (e) {} }
        function restore() {
            if (!restorePending) return;
            const saved = getSavedTime(), duration = val('duration');
        ...
        function resetWatchWindow() { playedSinceSeek = 0; lastTime = val('currentTime'); lastWallClock = Date.now(); }
        function markPlayingInterval() { lastTime = val('currentTime'); lastWallClock = Date.now(); }
        function showCompletionPanel() { const panel = document.querySelector('.mc-course-player__completion'); if (!panel) return; panel.hidden = false; panel.classList.add('is-visible'); }
        function submitCompletion() {
            if (completionSent || completedAt || !window.mathcoursePlayer || !mathcoursePlayer.ajax_url || !mathcoursePlayer.nonce) return;
            completionSent = true;
            const formData = new FormData();
            formData.append('action', 'mathcourse_complete_lesson'); formData.append('nonce', mathcoursePlayer.nonce); formData.append('lesson_id', String(lessonId));
            fetch(mathcoursePlayer.ajax_url, { method: 'POST', credentials: 'same-origin', body: formData, keepalive: true }).then(function (response) { if (!response.ok) throw new Error('HTTP ' + response.status); return response.json(); }).then(function (result) {
                if (!result || !result.success) throw new Error('progress rejected');
                completedAt = Date.now(); clearSavedTime(); const data = result.data || {}, serverCourseId = Number(data.course_id || courseId || 0);
                updateProgressUI(data.progress || null, lessonId); showCompletionPanel();
        ...
        on(player, 'timeupdate', function () {
            const time = val('currentTime'); if (!Number.isFinite(time)) return; savePosition(false);
            const now = Date.now(), elapsed = lastWallClock ? (now - lastWallClock) / 1000 : 0, delta = time - lastTime;
            if (!seeking && delta >= 0 && delta <= Math.max(2.5, elapsed + 1.5) && delta > 0) playedSinceSeek += Math.min(delta, 2.5);
            lastTime = time; lastWallClock = now;
        });
        ...
        // 只有真正播放到视频结束才记录课时完成；拖动进度条到末尾不会误标完成。
        on(player, 'ended', function () { submitCompletion(); });
        function persistBeforeLeave() { savePosition(true); }
        window.addEventListener('pagehide', persistBeforeLeave); window.addEventListener('beforeunload', persistBeforeLeave);
    });
});