document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.mathcourse-artplayer[data-video-url]').forEach(function (container) {
        ...
        function restorePosition(player) {
            ...
            try {
                player.currentTime = Math.min(saved, Math.max(0, duration - 1));
                restorePending = false;
            } catch (e) {}
        }
            ...
            fetch(mathcoursePlayer.ajax_url, {
                ...
                .then(function (response) {
                    ...
                    document.dispatchEvent(new CustomEvent('mathcourse_lesson_complete', {
                        detail: { lessonId: lessonId, courseId: cid, progress: data.progress || null }
                    }));
        ...
        function maybeCompleteAtEnd(player) {
            ...
            var time = Number(player.currentTime || 0);
            var duration = Number(player.duration || 0);
            if (Number.isFinite(time) && Number.isFinite(duration) && duration > 0 && time >= duration - Math.max(1, Math.min(5, duration * 0.01))) {
                submitCompletion(player);
            }
        }
