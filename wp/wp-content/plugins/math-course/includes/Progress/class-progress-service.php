...
class Progress_Service {
    ...
    public function is_completed($user_id,$lesson_id) {
        $user_id=absint($user_id); $lesson_id=absint($lesson_id);
        if(!$user_id||!$lesson_id)return false;
        if(in_array($lesson_id,$this->get_completed_lessons($user_id),true))return true;
        return $this->tutor->is_lesson_completed($lesson_id,$user_id);
    }

    public function get_last_completed_lesson($user_id,$course_id=0) {
        $completed=$this->get_completed_lessons($user_id);
        if(!$course_id)return !empty($completed)?(int)end($completed):0;
        $last=0;
        foreach($this->get_course_lessons(absint($course_id),false) as $lesson) {
            if($this->is_completed($user_id,(int)$lesson->ID))$last=(int)$lesson->ID;
        }
        return $last;
    }
    ...
    private function calculate_progress($course_id,$user_id,$include_unpublished=false) {
        ...
        $total=count($lessons); $completed=0; $last_lesson_id=0;
        foreach($lessons as $lesson) {
            $id=(int)$lesson->ID;
            if($this->is_completed($user_id,$id)) { $completed++; $last_lesson_id=$id; }
        }
        return array(
            'completed'=>$completed,
            'total'=>$total,
