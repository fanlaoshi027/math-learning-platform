...
class Progress_Ajax {
 ...
 public function complete(){
  ...
  $access=new Access_Service();$is_preview=$access->can_preview($course_id,$lesson_id);
  if(!$access->can_access_course($user_id,$course_id)&&!$is_preview)wp_send_json_error(array('message'=>'course access required'),403);
  $progress=new Progress_Service();$updated=$progress->complete_lesson($user_id,$lesson_id,true);
