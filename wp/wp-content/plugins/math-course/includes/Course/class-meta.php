...
class Meta {
	...
	public function add() {
		...
		add_meta_box( 'mathcourse_course_settings', 'MathCourse 课程信息', array( $this, 'box' ), tutor()->course_post_type, 'side', 'high' );
		add_meta_box( 'mathcourse_cover', '课程封面', array( $this, 'cover_box' ), tutor()->course_post_type, 'side', 'default' );
	}
	...
	public function box( $post ) {
		...
		?>
		<p><label for="mathcourse_type"><strong>课程类型</strong></label>
		...
		<p><label for="mathcourse_grade"><strong>年级</strong></label>
	...
	public function cover_box( $post ) {
		...
		?>
		<input type="url" name="mathcourse_cover" value="<?php echo esc_attr( $value ); ?>" placeholder="https://..." style="width:100%;">
		<p class="description">第一版先填写图片 URL，后续接入 WordPress 媒体库选择器。</p>
		<?php
	}

	public function save( $post_id, $post ) {
		if ( ! function_exists( 'tutor' ) || ! is_object( $post ) || tutor()->course_post_type !== $post->post_type ) {
			return;
		}
		...
		if ( ! isset( $_POST['mathcourse_course_meta_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mathcourse_course_meta_nonce'] ) ), 'mathcourse_course_meta' ) ) {
			return;
		}
