...
class Adapter
{
    ...
    public function get_course_tree($course_id)
    {
        ...
        $tree = array('course' => $course, 'topics' => array());
    ...
    public function create_course($title, $content = '', $status = 'draft')
    {
        if (!$this->is_available() || !$this->course_post_type()) {
            return new \WP_Error('mathcourse_tutor_unavailable', 'Tutor LMS 不可用。');
        }
        $title = sanitize_text_field($title);
        if ($title === '') return new \WP_Error('mathcourse_invalid_course_title', '课程名称不能为空。');
    ...
    {
        $course = $this->get_course($course_id);
        if (!$course) return new \WP_Error('mathcourse_invalid_course', 'Tutor 课程不存在。');
        $title = sanitize_text_field($title);
        if ($title === '') return new \WP_Error('mathcourse_invalid_topic_title', 'Topic 名称不能为空。');
        ...
        return is_wp_error($topic_id) ? $topic_id : get_post($topic_id);
    }
    ...
    public function create_lesson($topic_id, $title, $content = '', $page_number = 0)
    {
        ...
        if (!$topic_id || get_post_type($topic_id) !== $this->topic_post_type()) {
            return new \WP_Error('mathcourse_invalid_topic', 'Tutor Topic 不存在。');
        }
        $title = sanitize_text_field($title);
        if ($title === '') return new \WP_Error('mathcourse_invalid_lesson_title', 'Lesson 名称不能为空。');
        ...
        if (is_wp_error($lesson_id)) return $lesson_id;

        if ($page_number > 0) update_post_meta($lesson_id, 'page_number', absint($page_number));
    ...
    public function create_page_lessons($course_id, $topic_title, $start_page, $end_page)
    {
        ...
        if ($start_page < 1 || $end_page < $start_page) {
            return new \WP_Error('mathcourse_invalid_page_range', '页码范围无效。');
        }

        $topic = $this->create_topic($course_id, $topic_title);
        if (is_wp_error($topic)) return $topic;

        $lessons = array();
        for ($page = $start_page; $page <= $end_page; $page++) {
            $lesson = $this->create_lesson($topic->ID, '第 ' . $page . ' 页', '', $page);
            if (is_wp_error($lesson)) {
                return new \WP_Error(
                    'mathcourse_batch_lesson_failed',
                    '创建第 ' . $page . ' 页失败。',
                    array('topic_id' => $topic->ID, 'created_lessons' => count($lessons), 'error' => $lesson->get_error_message())
                );
            }
            $lessons[] = $lesson;
        }
        return array('course' => $course_id, 'topic' => $topic, 'lessons' => $lessons);
    }

    private function next_topic_order($course_id)
    {
        if (function_exists('tutor_utils') && method_exists(tutor_utils(), 'get_next_topic_order_id')) {
        ...
        return $orders ? ((int) get_post_field('menu_order', $orders[0]) + 1) : 0;
    }

    private function next_lesson_order($topic_id)
    {
        if (function_exists('tutor_utils') && method_exists(tutor_utils(), 'get_next_course_content_order_id')) {
