...
class Course_Directory {
    ...
    public function render($atts = array()) {
        $atts = shortcode_atts(array('course_id' => 0, 'course_type' => '', 'limit' => 0), $atts, 'mathcourse_course_directory');
        $course_id = absint($atts['course_id']);
        if (!$course_id && isset($_GET['course_id'])) $course_id = absint($_GET['course_id']);
        return $course_id ? $this->render_single_course($course_id) : $this->render_course_center($atts);
    }
    ...
    /** Public course directory. Presentation changes here never alter Tutor LMS's data model. */
    private function render_course_center($atts = array()) {
        ...
        $shortcode_type = !empty($atts['course_type']) ? sanitize_key($atts['course_type']) : '';
        $limit = !empty($atts['limit']) ? max(0, absint($atts['limit'])) : 0;
        if ($shortcode_type && in_array($shortcode_type, array('topic', 'supplementary'), true)) $type_filter = $shortcode_type;
        ...
        <div class="mathcourse-center">
            <?php if (!$shortcode_type) : ?>
            <div class="mc-course-filter" role="navigation" aria-label="课程筛选">
                <a class="<?php echo '' === $type_filter ? 'is-active' : ''; ?>" href="<?php echo esc_url(home_url('/course-center/')); ?>">全部</a>
                <a class="<?php echo 'topic' === $type_filter ? 'is-active' : ''; ?>" href="<?php echo esc_url(add_query_arg('course_type', 'topic', home_url('/course-center/'))); ?>">专题课程</a>
                <a class="<?php echo 'supplementary' === $type_filter ? 'is-active' : ''; ?>" href="<?php echo esc_url(add_query_arg('course_type', 'supplementary', home_url('/course-center/'))); ?>">教辅配套</a>
            </div>
            <?php endif; ?>
            <div class="mc-course-grade-filter" aria-label="年级筛选">
                ...
                <?php foreach (array(''=>'全部','7'=>'七年级','8'=>'八年级','9'=>'九年级') as $grade => $label) : ?>
                    ...
                    <a class="<?php echo (string)$grade_filter === (string)$grade ? 'is-active' : ''; ?>" href="<?php echo esc_url(add_query_arg($url_args, home_url('/course-center/'))); ?>"><?php echo esc_html($label); ?></a>
                <?php endforeach; ?>
            </div>
            <div class="mathcourse-center__grid">
                ...
                foreach ($courses as $course) :
                    ...
                    $lesson_count = 0;
                    foreach ($data['topics'] as $topic) $lesson_count += count($topic['lessons']);
                ?>
                    <article class="mathcourse-center__card" data-course-type="<?php echo esc_attr($data_type); ?>" data-course-grade="<?php echo esc_attr($data_grade); ?>">
                        <a class="mathcourse-center__cover" href="<?php echo esc_url($detail_url); ?>">
                            <?php if ($cover) : ?><img src="<?php echo esc_url($cover); ?>" alt="<?php echo esc_attr($data['title']); ?>" loading="lazy"><?php else : ?><span class="mathcourse-center__cover-placeholder">数学课程</span><?php endif; ?>
                        </a>
                        <div class="mathcourse-center__body">
                            <div class="mathcourse-center__meta">
                                ...
                                <span><?php echo esc_html($data_type === 'supplementary' ? '教辅配套' : '专题课程'); ?></span>
                                <span><?php echo esc_html($lesson_count); ?> 课时</span>
                            </div>
    ...
    public function render_learning_center() {
        ...
        $courses = get_posts(array('post_type' => function_exists('tutor') ? tutor()->course_post_type : 'courses','post_status'=>'publish','posts_per_page'=>-1,'orderby'=>array('menu_order'=>'ASC','date'=>'DESC')));
        $cards = array();
        foreach ($courses as $course) {
        ...
        <div class="mathcourse-learning-center">
            <div class="mathcourse-learning-center__heading"><div><p class="mc-eyebrow">MY LEARNING</p><h1>学习中心</h1><p>已授权课程与学习进度</p></div></div>
            <?php if (empty($cards)) : ?><div class="mathcourse-learning-center__empty"><strong>还没有已授权课程</strong><span>获得课程授权后，会显示在这里。</span></div><?php else : ?>
    ...
    private function find_continue_lesson($data) {
        ...
        $last = null;
        foreach ($data['topics'] as $topic) foreach ($topic['lessons'] as $lesson) { if ($lesson['completed']) $last = $lesson; elseif (!$last && $lesson['accessible']) return $lesson; }
        return $last;
    }
