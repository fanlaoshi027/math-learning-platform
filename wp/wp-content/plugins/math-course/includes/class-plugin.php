...
class Plugin
{
    ...
    private function load_modules()
    {
        ...
        $access_service = class_exists('MathCourse\\Access\\Access_Service')
            ? new Access\Access_Service() : null;

        if (class_exists('MathCourse\\Tutor\\Hooks')) new Tutor\Hooks();

        $video_service = class_exists('MathCourse\\Video\\Video_Service')
            ? new Video\Video_Service() : null;
        $token = class_exists('MathCourse\\Video\\Token') ? new Video\Token() : null;
        ...
        if ($video_service && $access_service && $token) {
            ...
            $storage = class_exists('MathCourse\\Video\\HLS_Storage') ? new Video\HLS_Storage() : null;
            ...
            if ($storage && class_exists('MathCourse\\Video\\Segment_Endpoints')) {
                (new Video\Segment_Endpoints($token, $video_service, $storage))->register();
            }
        }
    }
