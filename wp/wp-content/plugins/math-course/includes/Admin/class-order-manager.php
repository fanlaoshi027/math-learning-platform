...
class Order_Manager {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_mathcourse_save_order', array( $this, 'save_order' ) );
	}

	public function enqueue_assets() {
		if ( ! is_admin() || empty( $_GET['page'] ) || 'mathcourse-course-edit' !== sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			return;
		}
	...
	public function save_order() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => '无权操作。' ), 403 );
		}
		...
		foreach ( $topic_order as $position => $topic_id ) {
			$topic = get_post( $topic_id );
			if ( ! $topic || 'topics' !== $topic->post_type || (int) $topic->post_parent !== $course_id ) {
				continue;
			}
			wp_update_post( array( 'ID' => $topic_id, 'menu_order' => (int) $position ) );
		}

		foreach ( $lesson_order as $topic_id => $ids ) {
			$topic_id = absint( $topic_id );
			$topic    = get_post( $topic_id );
			if ( ! $topic || 'topics' !== $topic->post_type || (int) $topic->post_parent !== $course_id || ! is_array( $ids ) ) {
				continue;
			}
			foreach ( array_values( array_map( 'absint', $ids ) ) as $position => $lesson_id ) {
				$lesson = get_post( $lesson_id );
				if ( ! $lesson || tutor()->lesson_post_type !== $lesson->post_type || (int) $lesson->post_parent !== $topic_id ) {
					continue;
				}
