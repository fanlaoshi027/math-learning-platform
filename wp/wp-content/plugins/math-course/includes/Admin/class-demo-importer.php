...
class Demo_Importer {
    ...
    public function handle_import() {
        ...
        if (!empty($created)) {
            ...
            foreach ($created as $course_id) {
                $access->grant($user_id, $course_id, null);
            }
        }
    ...
    public function handle_remove() {
        ...
        foreach ($courses as $course_id) {
            ...
            if (!$course_id || 'yes' !== get_post_meta($course_id, '_mathcourse_demo', true)) {
                continue;
            }
        ...
        foreach (array('首页', '课程中心', '学习中心', '学习页') as $title) {
            ...
            if ($page && 'yes' === get_post_meta($page->ID, '_mathcourse_demo_page', true)) {
    ...
    private function definitions() {
        return array(
            array(
                'key' => 'algebra', 'title' => '代数', 'type' => 'topic', 'grade' => '7', 'cover' => 'algebra.svg',
                'topics' => array(
                    array('title' => '有理数', 'lessons' => array(
                        array('title' => '有理数的概念', 'trial' => true), array('title' => '有理数的运算'), array('title' => '数轴与绝对值'),
                    )),
                    array('title' => '整式', 'lessons' => array(
                        array('title' => '整式的概念'), array('title' => '整式的加减'), array('title' => '一元一次方程', 'trial' => true),
                    )),
                    array('title' => '一元一次方程', 'lessons' => array(
                        array('title' => '方程的概念'), array('title' => '解一元一次方程'), array('title' => '实际问题与方程'),
                    )),
                ),
            ),
            array(
                'key' => 'geometry', 'title' => '几何', 'type' => 'topic', 'grade' => '8', 'cover' => 'geometry.svg',
                'topics' => array(
                    array('title' => '线段与角', 'lessons' => array(
                        array('title' => '线段与角的基础', 'trial' => true), array('title' => '角的比较与运算'),
                    )),
                    array('title' => '三角形', 'lessons' => array(
                        array('title' => '三角形的概念'), array('title' => '三角形的边角关系'),
                    )),
                    array('title' => '全等三角形', 'lessons' => array(
                        array('title' => '全等三角形的概念与表示', 'trial' => true), array('title' => '边边边（SSS）'), array('title' => '边角边（SAS）'), array('title' => '角边角（ASA）'),
                    )),
                ),
            ),
            array(
                'key' => 'function', 'title' => '函数', 'type' => 'topic', 'grade' => '9', 'cover' => 'function.svg',
                'topics' => array(
                    array('title' => '一次函数', 'lessons' => array(
                        array('title' => '变量与函数', 'trial' => true), array('title' => '一次函数图象'), array('title' => '一次函数应用'),
                    )),
                    array('title' => '反比例函数', 'lessons' => array(
                        array('title' => '反比例函数概念'), array('title' => '反比例函数图象与性质'),
                    )),
                    array('title' => '二次函数', 'lessons' => array(
                        array('title' => '二次函数基础'), array('title' => '二次函数图象与性质'),
                    )),
                ),
            ),
            array(
                'key' => 'dapeiyou8', 'title' => '大培优·八年级上册', 'type' => 'supplementary', 'grade' => '8', 'cover' => 'dapeiyou.svg',
                'topics' => array(
                    array('title' => '大培优八上', 'lessons' => array(
                        array('title' => '第1页', 'page' => 1, 'trial' => true), array('title' => '第2页', 'page' => 2), array('title' => '第3页', 'page' => 3), array('title' => '第86页', 'page' => 86), array('title' => '第120页', 'page' => 120),
                    )),
                ),
            ),
        );
    ...
    private function find_or_create_topic($course_id, $title, $order) {
        ...
        foreach ($topics as $topic) {
            if ($topic->post_title === $title && 'yes' === get_post_meta($topic->ID, '_mathcourse_demo', true)) return (int) $topic->ID;
        }
    ...
    private function ensure_pages() {
        ...
        foreach ($pages as $title => $definition) {
            ...
            update_post_meta($page->ID, '_mathcourse_demo_page', 'yes');

            if ('首页' === $title && get_option('show_on_front') === 'page') update_option('page_on_front', $page->ID);
        }

        $course_page = get_page_by_path('course-center');
        $learning_center = get_page_by_path('learning-center');
        $learning_page = get_page_by_path('learning');
        if ($course_page) update_option('mathcourse_course_center_page_id', $course_page->ID);
        if ($learning_center) update_option('mathcourse_learning_center_page_id', $learning_center->ID);
        if ($learning_page) update_option('mathcourse_learning_page_id', $learning_page->ID);
    }
