            ...
            <div class="mathcourse-card">
                ...
                <?php if ( function_exists( 'tutor' ) ) : ?>
                    <table class="mathcourse-table">
                        ...
                        <?php endforeach; ?>
                        <?php if ( empty( $courses ) ) : ?><tr><td colspan="4">暂无课程，请先在 Tutor LMS 创建课程。</td></tr><?php endif; ?>
                        </tbody>
                    </table>
                <?php else : ?>
                    <p>Tutor LMS 尚未启用。启用 Tutor LMS 4.0.4 后，MathCourse 才能接管课程数据。</p>
                <?php endif; ?>
            </div>
        </div>
        <?php
