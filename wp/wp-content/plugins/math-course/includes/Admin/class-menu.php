    ...
    public function dashboard() {
        ...
        if ('POST' === ($_SERVER['REQUEST_METHOD'] ?? '') && isset($_POST['mathcourse_dashboard_action'])) {
            ...
            if ('grant_access' === $action && isset($_POST['mathcourse_dashboard_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mathcourse_dashboard_nonce'])), 'mathcourse_dashboard_grant')) {
                ...
                if (!$user) {
                    $message = '未找到该学员账号，请先在“学员授权管理”中创建账号。';
                    $message_type = 'error';
                } elseif (!$course || 'publish' !== $course->post_status) {
                    $message = '请选择有效的已发布课程。';
                    $message_type = 'error';
                } elseif ($access->grant($user->ID, $course_id)) {
                    $message = '已为“' . $user->display_name . '”开通课程授权。';
                } else {
                    $message = '课程授权保存失败，请稍后重试。';
                    $message_type = 'error';
                }
            }
        }

        $all_courses = $tutor->get_courses(true, -1);
        $published_courses = $tutor->get_courses(false, -1);
        ...
        $recent_courses = array_slice($all_courses, 0, 6);
        ?>
        <div class="wrap mathcourse-admin-wrap mathcourse-dashboard-page">
            <div class="mathcourse-dashboard-hero">
                <div>
                    <div class="mathcourse-eyebrow">MathCourse · 工作台</div>
                    <h1>樊老师数学课程中心</h1>
                    <p>课程、课时、学员授权与学习数据，都在这里统一管理。</p>
                </div>
                <div class="mathcourse-dashboard-hero-actions">
                    <a class="button mathcourse-primary" href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-courses&action=new')); ?>">＋ 新增课程</a>
                    <a class="button mathcourse-dashboard-secondary" href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-activation')); ?>">激活码管理</a>
                </div>
            </div>

            <?php if ($message !== '') : ?>
                <div class="notice mathcourse-dashboard-notice <?php echo 'error' === $message_type ? 'notice-error' : 'notice-success'; ?> is-dismissible"><p><?php echo esc_html($message); ?></p></div>
            <?php endif; ?>
            ...
            <div class="mathcourse-dashboard-main-grid">
                <section class="mathcourse-dashboard-panel mathcourse-dashboard-quick">
                    <div class="mathcourse-dashboard-panel-head">
                        <div><h2>⚡ 一键录入学员课程权限</h2><p>已有学员账号时，可直接从这里开通课程。</p></div>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-access')); ?>">学员管理 →</a>
                    </div>
                    <form method="post">
                        <input type="hidden" name="mathcourse_dashboard_action" value="grant_access">
                        <?php wp_nonce_field('mathcourse_dashboard_grant', 'mathcourse_dashboard_nonce'); ?>
                        <label>学员手机号</label>
                        <input type="text" name="student_phone" inputmode="tel" placeholder="输入学员手机号或账号">
                        <label>授权课程</label>
                        ...
                        <button type="submit" class="button mathcourse-primary mathcourse-dashboard-full-button">立即开通授权</button>
                    </form>
                </section>

                <section class="mathcourse-dashboard-panel mathcourse-dashboard-links">
                    <div class="mathcourse-dashboard-panel-head"><div><h2>常用管理</h2><p>把最常用的操作放在手边。</p></div></div>
                    <div class="mathcourse-dashboard-link-grid">
                        <a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-courses')); ?>"><span>📚</span><div><strong>课程与课时</strong><small>课程结构、视频、试看</small></div><b>→</b></a>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-access')); ?>"><span>👥</span><div><strong>学员授权</strong><small>课程权限与账号状态</small></div><b>→</b></a>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-activation')); ?>"><span>🎫</span><div><strong>激活码</strong><small>生成、分发与激活</small></div><b>→</b></a>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-progress')); ?>"><span>📈</span><div><strong>学习进度</strong><small>查看学员学习情况</small></div><b>→</b></a>
                    </div>
                </section>
            </div>

            <section class="mathcourse-dashboard-panel mathcourse-dashboard-courses">
                <div class="mathcourse-dashboard-panel-head"><div><h2>最近课程</h2><p>当前 MathCourse 中最近维护的课程。</p></div><a href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-courses')); ?>">查看全部课程 →</a></div>
                <?php if ($tutor->is_available() && !empty($recent_courses)) : ?>
                    <div class="mathcourse-dashboard-course-grid">
                        <?php foreach ($recent_courses as $course) :
                            $edit_url = add_query_arg(array('page' => 'mathcourse-course-edit', 'course_id' => $course->ID), admin_url('admin.php'));
                            $lesson_total = $tutor->get_course_lesson_count($course->ID);
                            $status_class = 'publish' === $course->post_status ? 'is-published' : ('private' === $course->post_status ? 'is-private' : 'is-draft');
                            ...
                            ?>
                            <a class="mathcourse-dashboard-course" href="<?php echo esc_url($edit_url); ?>">
                                <div class="mathcourse-dashboard-course-mark"><span><?php echo esc_html(mb_substr($course->post_title, 0, 1)); ?></span><i class="<?php echo esc_attr($status_class); ?>"></i></div>
                                <div class="mathcourse-dashboard-course-body"><strong><?php echo esc_html($course->post_title); ?></strong><div><span><?php echo esc_html($grade ?: '课程'); ?></span><span><?php echo esc_html($lesson_total); ?> 课时</span></div></div>
                                <span class="mathcourse-badge <?php echo esc_attr($status_class); ?>"><?php echo esc_html($status_label); ?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php elseif ($tutor->is_available()) : ?>
                    <div class="mathcourse-empty-state"><div class="mathcourse-empty-icon">📚</div><strong>还没有课程</strong><p>创建第一门课程后，这里会自动显示课程概况。</p><a class="button mathcourse-primary" href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-courses&action=new')); ?>">新增课程</a></div>
                <?php else : ?>
                    <div class="mathcourse-empty-state"><strong>Tutor LMS 尚未启用</strong><p>启用 Tutor LMS 4.0.4 后，MathCourse 才能接管课程数据。</p></div>
                <?php endif; ?>
            </section>
        </div>
        <?php
