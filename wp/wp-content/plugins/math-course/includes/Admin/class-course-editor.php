...
class Course_Editor {
	...
	private function render_lesson_editor( $course_id, $lesson ) {
		$preview = get_post_meta( $lesson->ID, '_mathcourse_preview', true ); $video = get_post_meta( $lesson->ID, '_mathcourse_video', true );
		?>
	...
	private function handle_actions( $course_id, &$message, &$message_type ) {
		...
		if ( 'save_lesson' === $action ) {
			...
			delete_post_meta( $lesson_id, '_mathcourse_page' ); update_post_meta( $lesson_id, '_mathcourse_preview', $preview ); update_post_meta( $lesson_id, '_mathcourse_video', $video ); $message = '课时已保存。'; return;
		}
