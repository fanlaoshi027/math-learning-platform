...
class Course_Editor {
			...
			<form method="post">
				...
				<div style="display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:20px;max-width:1180px;">
					<div style="background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:24px;">
						...
						<div style="margin-top:28px;padding-top:20px;border-top:1px solid #eee;"><h2 style="font-size:18px;margin:0;">课程内容</h2><p style="color:#646970;margin-top:6px;">按「专题 → 课时」组织。课时可以直接编辑名称、页码、简介、试看和发布状态。</p>
						...
						<?php $lesson_edit_url = add_query_arg( array( 'page' => 'mathcourse-course-edit', 'course_id' => $course_id, 'lesson_id' => $lesson->ID ), admin_url( 'admin.php' ) ); $preview = get_post_meta( $lesson->ID, '_mathcourse_preview', true ); $page = get_post_meta( $lesson->ID, '_mathcourse_page', true ); ?>
						<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid #f0f0f1;"><div><span style="display:inline-block;width:28px;color:#8c8f94;"><?php echo esc_html( $index + 1 ); ?>.</span><strong><?php echo esc_html( $lesson->post_title ); ?></strong><?php if ( $page ) : ?><span style="color:#646970;margin-left:8px;">页码 <?php echo esc_html( $page ); ?></span><?php endif; ?><?php if ( 'yes' === $preview ) : ?><span style="color:#2271b1;margin-left:8px;">试看</span><?php endif; ?></div><div style="display:flex;gap:10px;"><a href="<?php echo esc_url( $lesson_edit_url ); ?>">编辑</a><button type="submit" name="mathcourse_action" value="delete_lesson_<?php echo esc_attr( $lesson->ID ); ?>" class="button-link-delete" onclick="return confirm('确定删除这个课时吗？');">删除</button></div></div>
						<?php endforeach; ?><?php endif; ?><div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;"><input type="text" name="lesson_title_<?php echo esc_attr( $topic->ID ); ?>" placeholder="输入课时名称，例如：P1 三角形基础" style="min-width:320px;"><button type="submit" name="mathcourse_action" value="add_lesson_<?php echo esc_attr( $topic->ID ); ?>" class="button">＋ 添加课时</button></div></div></div>
					...
					<div><div style="background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:20px;"><h2 style="font-size:16px;margin-top:0;">课程属性</h2>
						<p><label for="course_type"><strong>课程类型</strong></label><select id="course_type" name="course_type" style="width:100%;margin-top:6px;"><option value="topic" <?php selected( $type, 'topic' ); ?>>专题课程</option><option value="supplementary" <?php selected( $type, 'supplementary' ); ?>>大培优配套</option></select></p>
	...
	private function render_lesson_editor( $course_id, $lesson ) {
		$page = get_post_meta( $lesson->ID, '_mathcourse_page', true ); $preview = get_post_meta( $lesson->ID, '_mathcourse_preview', true ); $video = get_post_meta( $lesson->ID, '_mathcourse_video', true );
		?>
		...
		<input type="hidden" name="editing_lesson_id" value="<?php echo esc_attr( $lesson->ID ); ?>"><div style="display:grid;grid-template-columns:2fr 1fr;gap:15px;margin-top:15px;"><p style="margin:0;"><label for="lesson_title_edit"><strong>课时名称</strong></label><input id="lesson_title_edit" name="lesson_title_edit" type="text" class="large-text" value="<?php echo esc_attr( $lesson->post_title ); ?>"></p><p style="margin:0;"><label for="lesson_page_edit"><strong>页码</strong></label><input id="lesson_page_edit" name="lesson_page_edit" type="text" value="<?php echo esc_attr( $page ); ?>" placeholder="P1"></p></div>
		<p><label for="lesson_description_edit"><strong>课时简介</strong></label><textarea id="lesson_description_edit" name="lesson_description_edit" rows="5" class="large-text"><?php echo esc_textarea( $lesson->post_content ); ?></textarea></p><p><label><input type="checkbox" name="lesson_preview_edit" value="yes" <?php checked( $preview, 'yes' ); ?>> <strong>允许试看</strong></label></p><p><label for="lesson_status_edit"><strong>状态</strong></label><select id="lesson_status_edit" name="lesson_status_edit"><option value="draft" <?php selected( $lesson->post_status, 'draft' ); ?>>草稿</option><option value="publish" <?php selected( $lesson->post_status, 'publish' ); ?>>已发布</option><option value="private" <?php selected( $lesson->post_status, 'private' ); ?>>私密</option></select></p><div style="padding-top:14px;border-top:1px solid #dbe7f0;"><strong>视频资源标识</strong><p style="color:#646970;margin:5px 0 8px;">本阶段仅保存资源标识，下一阶段接入 HLS / Token 鉴权。</p><input type="text" name="lesson_video_edit" value="<?php echo esc_attr( $video ); ?>" class="large-text" placeholder="视频 ID 或资源标识"></div><p><button type="submit" name="mathcourse_action" value="save_lesson" class="button button-primary">保存课时</button></p></div>
		<?php
	}
	...
	private function handle_actions( $course_id, &$message, &$message_type ) {
		...
		if ( 'save_lesson' === $action ) {
			...
			$title = isset( $_POST['lesson_title_edit'] ) ? sanitize_text_field( wp_unslash( $_POST['lesson_title_edit'] ) ) : $lesson->post_title; $content = isset( $_POST['lesson_description_edit'] ) ? wp_kses_post( wp_unslash( $_POST['lesson_description_edit'] ) ) : ''; $page = isset( $_POST['lesson_page_edit'] ) ? sanitize_text_field( wp_unslash( $_POST['lesson_page_edit'] ) ) : ''; $preview = isset( $_POST['lesson_preview_edit'] ) ? 'yes' : 'no'; $status = isset( $_POST['lesson_status_edit'] ) ? sanitize_key( wp_unslash( $_POST['lesson_status_edit'] ) ) : 'draft'; $video = isset( $_POST['lesson_video_edit'] ) ? sanitize_text_field( wp_unslash( $_POST['lesson_video_edit'] ) ) : '';
			...
			update_post_meta( $lesson_id, '_mathcourse_page', $page ); update_post_meta( $lesson_id, '_mathcourse_preview', $preview ); update_post_meta( $lesson_id, '_mathcourse_video', $video ); $message = '课时已保存。'; return;
		}
