...
<div class="wrap mathcourse-admin-wrap mathcourse-activation-admin">
 <div class="mathcourse-admin-header mathcourse-activation-header"><div><div class="mathcourse-admin-eyebrow">MathCourse · 学员服务</div><h1>激活码管理</h1><p>生成与课程绑定的一次性激活码，学员使用后自动获得对应课程授权。</p></div><a class="button mathcourse-primary" href="<?php echo esc_url(admin_url('admin.php?page=mathcourse-access')); ?>">学员授权 →</a></div>
 <div class="mathcourse-activation-stats">
  <div class="mathcourse-activation-stat"><div class="mathcourse-activation-stat-icon">全部</div><strong><?php echo esc_html($stat_total); ?></strong><span>全部激活码</span></div>
  <div class="mathcourse-activation-stat unused"><div class="mathcourse-activation-stat-icon">可用</div><strong><?php echo esc_html($stat_unused); ?></strong><span>未使用</span></div>
  <div class="mathcourse-activation-stat used"><div class="mathcourse-activation-stat-icon">已用</div><strong><?php echo esc_html($stat_used); ?></strong><span>已使用</span></div>
  <div class="mathcourse-activation-stat expired"><div class="mathcourse-activation-stat-icon">到期</div><strong><?php echo esc_html($stat_expired); ?></strong><span>已过期</span></div>
  <div class="mathcourse-activation-stat revoked"><div class="mathcourse-activation-stat-icon">撤销</div><strong><?php echo esc_html($stat_revoked); ?></strong><span>已撤销</span></div>
 </div>
 <section class="mathcourse-card mathcourse-activation-generator">
  <div class="mathcourse-card-title"><div><h2>生成激活码</h2><span class="mathcourse-card-subtitle">选择课程、数量和有效期后批量生成。</span></div></div>
 ...
 <section class="mathcourse-card mathcourse-generated-card">
  <div class="mathcourse-card-title"><div><h2>本次生成的激活码</h2><span class="mathcourse-card-subtitle">激活码只在这里显示一次，请复制保存后再发送给学员。</span></div><div class="mathcourse-generated-actions"><button type="button" class="button" id="mathcourse-copy-codes">复制全部</button><button type="button" class="button" id="mathcourse-download-codes">保存 TXT</button></div></div>
  <textarea class="mathcourse-code-output" id="mathcourse-code-output" rows="12" readonly><?php echo esc_textarea(implode("\n",$codes)); ?></textarea>
 </section>
 <?php endif; ?>
 <section class="mathcourse-card mathcourse-activation-list">
  <div class="mathcourse-card-title"><div><h2><?php echo $course_filter?'当前课程的激活码':'全部课程激活码'; ?></h2><span class="mathcourse-card-subtitle">当前列表 <?php echo esc_html($total); ?> 条</span></div></div>
  <div class="mathcourse-activation-toolbar">
   <form method="get"><input type="hidden" name="page" value="mathcourse-activation"><select name="course_id"><option value="0">全部课程</option><?php foreach($courses as $course): ?><option value="<?php echo esc_attr($course->ID); ?>" <?php selected($course_filter,$course->ID); ?>><?php echo esc_html($course->post_title); ?></option><?php endforeach; ?></select><select name="status"><option value="">全部状态</option><option value="unused" <?php selected($status,'unused'); ?>>未使用</option><option value="used" <?php selected($status,'used'); ?>>已使用</option><option value="expired" <?php selected($status,'expired'); ?>>已过期</option><option value="revoked" <?php selected($status,'revoked'); ?>>已撤销</option></select><button class="button" type="submit">筛选</button></form>
   <?php if(!empty($courses)): ?><div class="mathcourse-activation-quick-courses"><span>快速进入：</span><?php foreach($courses as $course): ?><a class="button" href="<?php echo esc_url(add_query_arg(array('page'=>'mathcourse-activation','course_id'=>$course->ID),admin_url('admin.php'))); ?>"><?php echo esc_html($course->post_title); ?></a><?php endforeach; ?></div><?php endif; ?>
  </div>
  ...
  <?php if(empty($rows)): ?><tr><td colspan="8"><div class="mathcourse-empty-state"><strong>暂无符合条件的激活码</strong><p>调整课程或状态筛选后再试。</p></div></td></tr><?php else: foreach($rows as $row): $course=get_post($row->course_id);$user=$row->used_by?get_user_by('id',$row->used_by):false;$labels=array('unused'=>'未使用','used'=>'已使用','expired'=>'已过期','revoked'=>'已撤销');$label=$labels[$row->status]??$row->status; ?>
  <tr><td><?php echo esc_html($row->id); ?></td><td><strong><?php echo esc_html($course?$course->post_title:'课程已删除'); ?></strong></td><td><span class="mathcourse-badge <?php echo esc_attr($row->status); ?>"><?php echo esc_html($label); ?></span></td><td><?php echo esc_html($row->created_at); ?></td><td><?php echo esc_html($row->expires_at?$row->expires_at:'不限'); ?></td><td><?php echo esc_html($user?$user->user_login:'—'); ?></td><td><?php echo esc_html($row->used_at?$row->used_at:'—'); ?></td><td><?php if('unused'===$row->status): ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('确定撤销这个激活码吗？撤销后不能再使用。');"><?php wp_nonce_field('mathcourse_revoke_code_'.$row->id); ?><input type="hidden" name="action" value="mathcourse_revoke_code"><input type="hidden" name="code_id" value="<?php echo esc_attr($row->id); ?>"><button class="button-link-delete" type="submit">撤销</button></form><?php else: ?>—<?php endif; ?></td></tr>
