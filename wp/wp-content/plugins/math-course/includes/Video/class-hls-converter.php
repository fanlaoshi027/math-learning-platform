 ...
 * 后台 MP4 → HLS 转换器。
 *
 * 只做无损封装/切片（-c copy），不重新编码视频，因此速度主要取决于磁盘读写。
 */
class Hls_Converter {
    ...
    public function ajax_upload() {
        ...
        if ( ! $lesson_id || ! $course_id || ! $this->adapter->get_lesson( $lesson_id ) ) wp_send_json_error( array( 'message' => '课时信息无效。' ), 400 );
        ...
        if ( is_wp_error( $info ) ) {
            @unlink( $source );
            wp_send_json_error( array( 'message' => $info->get_error_message() ), 400 );
        }
    ...
    public function ajax_status() {
        ...
        wp_send_json_success( array(
            'status' => get_post_meta( $lesson_id, self::META_STATUS, true ) ?: 'none',
            'hls_url' => get_post_meta( $lesson_id, self::META_HLS, true ),
            'error' => get_post_meta( $lesson_id, self::META_ERROR, true ),
            'info' => get_post_meta( $lesson_id, self::META_INFO, true ),
        ) );
    }
    ...
    public function convert( $lesson_id ) {
        ...
        if ( ! $source || ! $course_id || ! is_file( $source ) ) {
            update_post_meta( $lesson_id, self::META_STATUS, 'failed' );
            update_post_meta( $lesson_id, self::META_ERROR, '找不到上传的 MP4 文件。' );
            return;
        }

        update_post_meta( $lesson_id, self::META_STATUS, 'processing' );
        $info = $this->probe( $source );
        if ( is_wp_error( $info ) ) return $this->fail( $lesson_id, $info->get_error_message() );
        update_post_meta( $lesson_id, self::META_INFO, $info );

        $video_codec = strtolower( (string) ( $info['video_codec'] ?? '' ) );
        $audio_codec = strtolower( (string) ( $info['audio_codec'] ?? '' ) );
        if ( ! in_array( $video_codec, array( 'h264', 'hevc' ), true ) ) return $this->fail( $lesson_id, '该 MP4 的视频编码为 ' . ( $video_codec ?: '未知' ) . '，不能安全地直接切 HLS。请使用 H.264 视频编码后再上传。' );
        if ( $audio_codec && 'aac' !== $audio_codec ) return $this->fail( $lesson_id, '该 MP4 的音频编码为 ' . $audio_codec . '，不能安全地直接切 HLS。请使用 AAC 音频后再上传。' );

        $hls_dir = $this->get_hls_dir( $course_id, $lesson_id );
        if ( ! wp_mkdir_p( $hls_dir ) || ! is_writable( $hls_dir ) ) return $this->fail( $lesson_id, 'HLS 输出目录不可写。' );
        ...
        if ( ! is_executable( $ffmpeg ) ) return $this->fail( $lesson_id, '找不到可执行的 FFmpeg：' . $ffmpeg );

        $cmd = escapeshellarg( $ffmpeg ) . ' -hide_banner -loglevel error -y -i ' . escapeshellarg( $source ) . ' -map 0:v:0 -map 0:a? -c copy -start_number 0 -hls_time 8 -hls_list_size 0 -hls_segment_filename ' . escapeshellarg( $pattern ) . ' -f hls ' . escapeshellarg( $playlist ) . ' 2>&1';
        $output = array();
        $exit = 0;
        @set_time_limit( 0 );
        exec( $cmd, $output, $exit );
        if ( 0 !== $exit || ! is_file( $playlist ) || filesize( $playlist ) < 20 ) return $this->fail( $lesson_id, 'FFmpeg HLS 转换失败：' . trim( implode( "\n", array_slice( $output, -8 ) ) ) );

        $relative = '/__mathcourse_hls/course-' . $course_id . '/lesson-' . $lesson_id . '/index.m3u8';
        update_post_meta( $lesson_id, self::META_HLS, $relative );
        update_post_meta( $lesson_id, self::META_STATUS, 'ready' );
        delete_post_meta( $lesson_id, self::META_ERROR );
    }

    private function fail( $lesson_id, $message ) {
        update_post_meta( $lesson_id, self::META_STATUS, 'failed' );
        update_post_meta( $lesson_id, self::META_ERROR, sanitize_textarea_field( (string) $message ) );
    }
    ...
    private function get_upload_root() {
        if ( defined( 'MATHCOURSE_VIDEO_UPLOAD_ROOT' ) && MATHCOURSE_VIDEO_UPLOAD_ROOT ) return untrailingslashit( MATHCOURSE_VIDEO_UPLOAD_ROOT );
        if ( defined( 'MATHCOURSE_MEDIA_ROOT' ) && MATHCOURSE_MEDIA_ROOT ) return dirname( untrailingslashit( MATHCOURSE_MEDIA_ROOT ) ) . '/uploads';
        return WP_CONTENT_DIR . '/uploads/mathcourse-video-source';
    }

    private function get_hls_dir( $course_id, $lesson_id ) {
        $root = defined( 'MATHCOURSE_MEDIA_ROOT' ) ? untrailingslashit( MATHCOURSE_MEDIA_ROOT ) : WP_CONTENT_DIR . '/uploads/mathcourse-hls';
        return $root . '/course-' . absint( $course_id ) . '/lesson-' . absint( $lesson_id );
    }
    ...
    private function remove_dir( $dir ) {
        ...
        foreach ( $items as $item ) { if ( '.' === $item || '..' === $item ) continue; $path = $dir . '/' . $item; if ( is_dir( $path ) ) $this->remove_dir( $path ); else @unlink( $path ); }
        @rmdir( $dir );
    }
}
