...
class Hls_Converter {
    ...
    public function convert( $lesson_id ) {
        ...
        if ( ! in_array( $video_codec, array( 'h264', 'hevc' ), true ) ) { $this->fail( $lesson_id, '该 MP4 的视频编码为 ' . ( $video_codec ?: '未知' ) . '，不能安全地直接切 HLS。请使用 H.264 视频编码后再上传。' ); return; }
        if ( $audio_codec && 'aac' !== $audio_codec ) { $this->fail( $lesson_id, '该 MP4 的音频编码为 ' . $audio_codec . '，不能安全地直接切 HLS。请使用 AAC 音频后再上传。' ); return; }
        $hls_dir = $this->get_hls_dir( $course_id, $lesson_id );
