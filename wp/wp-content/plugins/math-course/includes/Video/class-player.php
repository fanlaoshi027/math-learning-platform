...
class Player {
    ...
    public function assets() {
        // ArtPlayer 自带官方播放器 UI；这里只加载 HLS 解码器和业务脚本。
        wp_enqueue_script( 'mathcourse-hls', 'https://cdn.jsdelivr.net/npm/hls.js@1.7.0/dist/hls.min.js', array(), '1.7.0', true );
        wp_enqueue_script( 'mathcourse-artplayer', 'https://cdn.jsdelivr.net/npm/artplayer@5.4.0/dist/artplayer.js', array(), '5.4.0', true );
    ...
    public function render( $atts ) {
        ...
        if ( ! empty( $video['hls_url'] ) ) {
            $this->assets();
            ob_start(); ?>
            <div id="mathcourse-player-<?php echo esc_attr( $lesson_id ); ?>"
                 class="mathcourse-artplayer"
                 data-video-url="<?php echo esc_attr( $video['hls_url'] ); ?>"
                 data-lesson-id="<?php echo esc_attr( $lesson_id ); ?>"
                 data-course-id="<?php echo esc_attr( $course_id ); ?>"
                 aria-label="课程视频播放器"></div>
