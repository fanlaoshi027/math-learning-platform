...
function mc_theme_assets() {
	...
	if ( is_page( 'learning' ) || is_page_template( 'page-learning.php' ) ) {
		...
		if ( file_exists( $player_ui_path ) && ! is_page( 'learning-center' ) ) {
			wp_enqueue_style( 'mc-course-player-ui-v1', get_stylesheet_directory_uri() . '/assets/css/course-player-ui-v1.css', array( 'mc-learning-ui-polish' ), (string) filemtime( $player_ui_path ) );
		}
		wp_enqueue_style( 'mc-learning-v2', get_stylesheet_directory_uri() . '/assets/css/learning-v2.css', array( 'mc-learning-ui-polish', 'mc-course-player-ui-v1', 'mc-ui-reference-override' ), $learning_v2_version );
	}
}
...
function mc_route_course_to_learning_player() {
	...
	if ( $lesson_id ) { $args['lesson_id'] = $lesson_id; }
