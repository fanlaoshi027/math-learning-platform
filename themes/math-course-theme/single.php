...
while ( have_posts() ) : the_post();
	$post_id = get_the_ID();
	$adapter = new Adapter();
	$is_lesson = $adapter->is_available() && $adapter->get_lesson_post_type() === get_post_type( $post_id );
	?>
			...
			<?php if ( $is_lesson ) : ?>
				...
				<article class="mc-lesson-page" data-lesson-id="<?php echo esc_attr( $post_id ); ?>" data-course-id="<?php echo esc_attr( $course_id ); ?>">
					...
					<header class="mc-lesson-header">
						<h1><?php the_title(); ?></h1>
						<?php if ( ! empty( $video['preview'] ) ) : ?><span class="mc-lesson-badge">试看</span><?php endif; ?>
					</header>
