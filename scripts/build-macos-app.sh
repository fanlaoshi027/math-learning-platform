...
renderer=Path('Sources/MosuanBoard/Metal/InkRenderer.swift'); s=renderer.read_text()
...
metal_view=Path('Sources/MosuanBoard/Metal/InkMetalView.swift'); s=metal_view.read_text(); s=s.replace('colorPixelFormat = .bgra8Unorm; clearColor','colorPixelFormat = .bgra8Unorm; isOpaque = false; layer?.isOpaque = false; clearColor',1); metal_view.write_text(s)
PY
...
ARCH="$(uname -m)"
file "$BINARY"
if [ "$ARCH" = "arm64" ] && ! file "$BINARY" | grep -q "arm64"; then exit 1; fi
cp "$BINARY" "$MACOS_DIR/MosuanBoard"
RESOURCE_BUNDLE="$(find "$ROOT_DIR/.build" -type d -name 'MosuanBoard_MosuanBoard.bundle' -print -quit)"
[ -n "$RESOURCE_BUNDLE" ] || exit 1
METAL_SOURCE="$ROOT_DIR/Sources/MosuanBoard/Metal/InkShaders.metal"
METAL_BUILD_DIR="$ROOT_DIR/dist/metal-build"; AIR="$METAL_BUILD_DIR/InkShaders.air"; METALLIB="$METAL_BUILD_DIR/default.metallib"
...
ICONSET="$ROOT_DIR/dist/AppIcon.iconset"; mkdir -p "$ICONSET"
...
# Sign the nested executable and the resource bundle first, then sign the app itself.
# Do not use --deep here: deep signing can rewrite nested code/resources unexpectedly.
codesign --force --sign - "$MACOS_DIR/MosuanBoard"
codesign --force --sign - "$RESOURCES_DIR/MosuanBoard_MosuanBoard.bundle"
codesign --force --sign - "$APP_DIR"
codesign --verify --deep --strict --verbose=2 "$APP_DIR"
DMG="$ROOT_DIR/dist/Mosuan-Board-${VERSION}.dmg"; DMG_STAGING="$ROOT_DIR/dist/dmg-staging"; rm -rf "$DMG_STAGING"; mkdir -p "$DMG_STAGING"; cp -R "$APP_DIR" "$DMG_STAGING/"; ln -s /Applications "$DMG_STAGING/Applications"
...
echo "Built: $APP_DIR"; echo "Built: $DMG"; echo "Architecture: $ARCH"; echo "Version: $VERSION"; echo "Build: $BUILD_NUMBER"