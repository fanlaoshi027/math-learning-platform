...
struct BoardScreen: View {
    ...
    private var toolbarAlignment: Alignment {
        switch toolbarDock { case .top: return .top; case .bottom: return .bottom; case .left: return .leading; case .right: return .trailing }
    }

    @ViewBuilder private func toolbar(in size: CGSize) -> some View {
        if toolbarIsVertical {
            VStack(spacing: 8) { dragHandle(in: size); Divider(); toolbarContents }.padding(8)
        } else {
            HStack(spacing: 10) { dragHandle(in: size); Divider().frame(height: 24); toolbarContents }
    ...
    @ViewBuilder private var toolbarContents: some View {
        ...
        if toolbarIsVertical { Divider() } else { Divider().frame(height: 24) }

        ForEach(PenPreset.defaults) { item in
            Button { presetID = item.id; tool = .pen } label: {
                Circle().fill(Color(red: item.style.color.red, green: item.style.color.green, blue: item.style.color.blue))
        ...
        if controller.hasSelection {
            if toolbarIsVertical {
                Divider()
                VStack(spacing: 4) { Image(systemName: "rotate.right"); TextField("角度", text: rotationBinding).frame(width: 56).textFieldStyle(.roundedBorder).onSubmit { applyRotation() }; Text("°") }
                    .help("输入旋转角度")
            } else {
    ...
    private func dragHandle(in size: CGSize) -> some View {
        Image(systemName: "line.3.horizontal").font(.system(size: 14, weight: .semibold)).frame(width: 28, height: 32).contentShape(Rectangle()).foregroundStyle(.secondary)
            .help("拖动工具条到上、下、左、右")
            .gesture(DragGesture(coordinateSpace: .named("board"))
                .onChanged { value in isDraggingToolbar = true; dragLocation = value.location }
                .onEnded { value in toolbarDock = nearestDock(for: value.location, in: size); isDraggingToolbar = false })
    }
    ...
    private func edgeGuide(_ dock: ToolbarDock, selected: Bool, size: CGSize) -> some View {
        ...
        return Rectangle().fill(Color.accentColor.opacity(selected ? 0.28 : 0.08))
            .frame(width: dock == .left || dock == .right ? thickness : size.width, height: dock == .top || dock == .bottom ? thickness : size.height)
            .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: edgeAlignment(dock))
            .overlay(alignment: edgeAlignment(dock)) { if selected { Text("停靠").font(.caption2.weight(.semibold)).foregroundStyle(Color.accentColor).padding(.horizontal, 8).padding(.vertical, 4).background(.regularMaterial, in: Capsule()).padding(12) } }
    }

    private func edgeAlignment(_ dock: ToolbarDock) -> Alignment {
        switch dock { case .top: return .top; case .bottom: return .bottom; case .left: return .leading; case .right: return .trailing }
    }

    private func nearestDock(for point: CGPoint, in size: CGSize) -> ToolbarDock {
        let distances: [(ToolbarDock, CGFloat)] = [(.top, max(0, point.y)), (.bottom, max(0, size.height - point.y)), (.left, max(0, point.x)), (.right, max(0, size.width - point.x))]
        return distances.min(by: { $0.1 < $1.1 })?.0 ?? .top
    }

    private func applyRotation() {
        guard let value = Double(rotationText) else { rotationText = String(format: "%.1f", controller.rotationDegrees); return }
