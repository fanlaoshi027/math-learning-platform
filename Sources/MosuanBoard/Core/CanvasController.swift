...
final class CanvasController: ObservableObject {
    ...
    func attach(_ canvas: InkMetalView) {
        ...
        canvas.onHistoryChanged = { [weak self, weak canvas] in
            ...
            self.canUndo = canvas.canUndo; self.canRedo = canvas.canRedo
        }
        canvas.onSelectionChanged = { [weak self, weak canvas] in
            guard let self, let canvas else { return }
            self.hasSelection = canvas.hasSelection; self.selectionCount = canvas.selectionCount; self.rotationDegrees = canvas.selectedRotationDegrees
        }
        refreshState()
    }
    func undo(){canvas?.undo();refreshState()};func redo(){canvas?.redo();refreshState()};func deleteSelected(){canvas?.deleteSelected();refreshState()}
    ...
    func refreshState(){canUndo=canvas?.canUndo ?? false;canRedo=canvas?.canRedo ?? false;hasSelection=canvas?.hasSelection ?? false;selectionCount=canvas?.selectionCount ?? 0;rotationDegrees=canvas?.selectedRotationDegrees ?? 0}
}
