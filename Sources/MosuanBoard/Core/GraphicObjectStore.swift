...
final class GraphicObjectStore {
    ...
    func addLine(from start: CGPoint, to end: CGPoint, style: GraphicObject.Style) -> UUID {
        let object = GraphicObject.line(from: start, to: end, style: style); objects.append(object); return object.id
    }
    @discardableResult
    func addPolygon(points: [CGPoint], style: GraphicObject.Style) -> UUID {
        let object = GraphicObject.polygon(points: points, style: style); objects.append(object); return object.id
    }
    @discardableResult
    func addDynamicAngle(vertex: CGPoint, startLength: CGFloat = 180, endLength: CGFloat = 180, angleDegrees: CGFloat = 45, minimum: CGFloat = 10, maximum: CGFloat = 170, step: CGFloat = 1, name: String = "α", style: GraphicObject.Style = GraphicObject.Style()) -> UUID {
        let object = DynamicAngleFactory.makeGraphicObject(vertex: vertex, startLength: startLength, endLength: endLength, angleDegrees: angleDegrees, minimum: minimum, maximum: maximum, step: step, name: name, style: style); objects.append(object); return object.id
    }
    @discardableResult
    ...
    @discardableResult
    func dragDynamicTriangleLegLengthControl(id: UUID, to point: CGPoint) -> Bool {
        guard let i = objects.firstIndex(where: { $0.id == id }), objects[i].kind == .parameterizedTriangle, var model = objects[i].triangleModel else { return false }
        let rawPoint = inverseTransform(point, in: objects[i])
        guard let length = model.legLength(forControlPoint: rawPoint), length.isFinite else { return false }
        model.setLegLength(length)
        objects[i].triangleModel = model
        objects[i].geometry.points = model.vertices()
        return true
    ...
    private func inverseTransform(_ point: CGPoint, in object: GraphicObject) -> CGPoint {
        let p = CGPoint(x: point.x - object.transform.position.x - object.transform.rotationCenter.x,
                        y: point.y - object.transform.position.y - object.transform.rotationCenter.y)
        let co = cos(object.transform.rotation)
        let si = sin(object.transform.rotation)
