...
/// Controls how a geometric point is presented on the canvas.
enum GeometryPointLabelMode: String, Codable, CaseIterable {
    case hidden
    case name
}

/// Presentation settings for a named/pinned point.
/// The offset is user-adjustable, while autoAvoid lets the engine choose a
/// better nearby position during dynamic movement.
struct GeometryPointLabel: Codable, Equatable {
    ...
    init(
        mode: GeometryPointLabelMode = .name,
        text: String = "A",
...
/// The final label placement calculated for a frame of dynamic geometry.
...
/// Shared, platform-independent label placement engine.
/// It intentionally works only with geometry primitives so macOS/iPadOS/
/// Windows can use exactly the same avoidance behaviour.
enum GeometryLabelAvoidance {
    ...
    static func placement(
        ...
        guard label.autoAvoid else {
            let rect = CGRect(center: preferred, size: size)
            return GeometryLabelPlacement(center: preferred, bounds: rect)
        }

        // Prefer the teacher's chosen direction, then rotate around the point.
        ...
        for (index, angle) in angles.enumerated() {
            ...
            let score = score(rect: rect, point: point, obstacles: obstacles, otherLabels: otherLabels)
                + CGFloat(index) * 0.01
    ...
    private static func score(
        ...
        for obstacle in obstacles {
            ...
            if expanded.intersects(lineBounds(obstacle.segmentStart, obstacle.segmentEnd)) {
                ...
                if distance < obstacle.padding {
                    score += (obstacle.padding - distance + 1) * 120
                } else {
                    score += max(0, obstacle.padding * 2 - distance) * 10
                }
            }
        }

        for other in otherLabels where rect.intersects(other.insetBy(dx: -4, dy: -4)) {
            score += 2_000
        }

        // Slightly prefer labels that stay close to their point.
