...
extension GraphicObjectStore {
    ...
    @discardableResult
    func moveToFront(ids: [UUID]) -> Bool { reorder(ids: ids, direction: .front) }
    @discardableResult
    func moveToBack(ids: [UUID]) -> Bool { reorder(ids: ids, direction: .back) }
    @discardableResult
    func moveForward(ids: [UUID]) -> Bool { reorder(ids: ids, direction: .forward) }
    @discardableResult
    func moveBackward(ids: [UUID]) -> Bool { reorder(ids: ids, direction: .backward) }
    ...
    private func reorder(ids: [UUID], direction: ReorderDirection) -> Bool {
        let wanted = Set(ids)
        guard !wanted.isEmpty, objects.count > 1 else { return false }
        ...
        case .forward:
            ...
            for index in stride(from: objects.count - 2, through: 0, by: -1) where wanted.contains(objects[index].id) {
                ...
                if next < objects.count { objects.swapAt(index, next) }
            }
        case .backward:
            for index in 1..<objects.count where wanted.contains(objects[index].id) {
                ...
                if previous >= 0 { objects.swapAt(index, previous) }
            }
        }
