...
enum BoardTool: Equatable { case select, pen, line, smartLine, polygon, dynamicAngle, eraser }
...
enum BoardPattern: Int, CaseIterable, Identifiable {
    ...
    var id: Int { rawValue }
    var title: String {
        switch self { case .blank: "空白"; case .ruled: "横线"; case .grid: "方格"; case .dots: "点阵"; case .mathGrid: "数学方格" }
    }
}
...
enum EyeComfortBackground: String, CaseIterable, Identifiable {
    ...
    var color: SIMD4<Float> {
        switch self { case .black90: SIMD4(0.10,0.10,0.10,1); case .deepGreen: SIMD4(0.08,0.16,0.12,1); case .deepBlue: SIMD4(0.07,0.12,0.22,1); case .custom: SIMD4(0.10,0.10,0.10,1) }
    }
}
...
struct ToolButton: View {
    ...
    let action: () -> Void
    var body: some View {
        Button(action: action) { Label(title, systemImage: systemImage).padding(.horizontal, 8) }
...
struct PageSidebar: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Button { controller.addPage() } label: { Image(systemName: "plus") }.buttonStyle(.borderless)
            }
            .padding(.horizontal, 10).padding(.top, 10)
            ScrollView {
                LazyVStack(spacing: 8) {
                    ForEach(Array(controller.pages.enumerated()), id: \.element.id) { index, page in
                        Button { controller.selectPage(index) } label: {
                            VStack(spacing: 4) {
                                RoundedRectangle(cornerRadius: 5)
                                    .fill(pagePreviewColor(page.background))
                                    .overlay {
                                        RoundedRectangle(cornerRadius: 5)
                                            .stroke(controller.currentIndex == index ? Color.accentColor : Color.gray.opacity(0.25), lineWidth: controller.currentIndex == index ? 2 : 1)
                                    }
                                    .overlay { Text("第 \(index + 1) 页").font(.system(size: 11, weight: .medium)).foregroundStyle(page.background == "black" ? .white : .primary) }
                                    .frame(width: 120, height: 76)
                                Text(page.title).font(.system(size: 10)).foregroundStyle(.secondary).lineLimit(1)
                            }
                            .frame(width: 128)
                        }
                        .buttonStyle(.plain)
            ...
            Spacer()
            HStack {
                Button { controller.duplicateCurrentPage() } label: { Image(systemName: "plus.square.on.square") }
                Button(role: .destructive) { controller.deleteCurrentPage() } label: { Image(systemName: "trash") }
            }
            .padding(.bottom, 10)
        }
        .frame(width: 145)
    ...
    private func pagePreviewColor(_ value: String) -> Color {
        switch value { case "black": .black; case "darkGray": Color(white: 0.18); case "lightGray": Color(white: 0.92); case "cream": Color(red: 0.98, green: 0.96, blue: 0.88); default: .white }
    }
}
