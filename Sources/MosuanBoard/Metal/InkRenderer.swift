...
final class InkRenderer: NSObject, MTKViewDelegate {
    ...
    var canUndo:Bool{!undoStack.isEmpty};var canRedo:Bool{!redoStack.isEmpty};var hasSelection:Bool{!selectedStrokeIndices.isEmpty || !selectedObjectIDs.isEmpty};var selectionCount:Int{selectedStrokeIndices.count+selectedObjectIDs.count};var selectedStrokeIndex:Int?{selectedObjectIDs.isEmpty && selectedStrokeIndices.count==1 ? selectedStrokeIndices[0]:nil};var zoomPercent:Int{Int((zoomScale*100).rounded())}
    ...
    func exportPageState()->CanvasPageState{return CanvasPageState(strokes:committedStrokes.map{CanvasStroke(id:$0.id,points:$0.points,style:$0.style,rotation:$0.rotation)},objects:objectStore.exportObjects())}
    ...
    private func recordMutation(){guard transactionStart==nil else{return};undoStack.append(captureState());redoStack.removeAll()};private func captureState()->HistoryState{HistoryState(strokes:committedStrokes,objects:objectStore.exportObjects(),selection:selectedStrokeIndices,objectSelection:selectedObjectIDs)};private func restore(_ s:HistoryState){committedStrokes=s.strokes;objectStore.importObjects(s.objects);selectedStrokeIndices=s.selection.filter{committedStrokes.indices.contains($0)};selectedObjectIDs=s.objectSelection.filter{objectStore.object(with:$0) != nil};customRotationCenter=nil;rebuildGeometry()}
    ...
    func commitStroke(_ p:[InkPoint]){guard p.count>=2 else{activeStroke=[];rebuildGeometry();return};recordMutation();committedStrokes.append(StoredStroke(id:UUID(),points:p,style:penStyle));selectedStrokeIndices=[];selectedObjectIDs=[];activeStroke=[];rebuildGeometry()}

    /// Commits a straight line as a structured GraphicObject so it stays editable.
    func commitLine(from start: SIMD2<Float>, to end: SIMD2<Float>){
        ...
        _=objectStore.addLine(from:CGPoint(x:CGFloat(start.x),y:CGFloat(start.y)),to:CGPoint(x:CGFloat(end.x),y:CGFloat(end.y)),style:style)
        selectedStrokeIndices=[];selectedObjectIDs=[];activeStroke=[];rebuildGeometry()
    }

    @discardableResult func selectObject(at point:SIMD2<Float>,tolerance:Float=10)->Bool{
        let c=canvasPoint(from:point)
        guard let id=objectStore.hitTest(at:CGPoint(x:CGFloat(c.x),y:CGFloat(c.y)),tolerance:CGFloat(tolerance/zoomScale)) else {selectedObjectIDs=[];selectedStrokeIndices=[];customRotationCenter=nil;rebuildGeometry();return false}
    ...
    @discardableResult func toggleObject(at point:SIMD2<Float>,tolerance:Float=10)->Bool{
        let c=canvasPoint(from:point)
        guard let id=objectStore.hitTest(at:CGPoint(x:CGFloat(c.x),y:CGFloat(c.y)),tolerance:CGFloat(tolerance/zoomScale)) else{return false}
        if selectedObjectIDs.contains(id){selectedObjectIDs.removeAll{$0==id}}else{selectedObjectIDs.append(id)}
        selectedStrokeIndices=[];customRotationCenter=nil;rebuildGeometry();return true
    }

    @discardableResult func selectObjects(in viewRect:CGRect,fullyContained:Bool=false)->Int{
        let p0=canvasPoint(from:SIMD2(Float(viewRect.minX),Float(viewRect.minY))),p1=canvasPoint(from:SIMD2(Float(viewRect.maxX),Float(viewRect.maxY)))
        let r=CGRect(x:CGFloat(min(p0.x,p1.x)),y:CGFloat(min(p0.y,p1.y)),width:CGFloat(abs(p1.x-p0.x)),height:CGFloat(abs(p1.y-p0.y)))
        selectedObjectIDs=objectStore.objectsIntersecting(r,fullyContained:fullyContained);selectedStrokeIndices=[];customRotationCenter=nil;rebuildGeometry();return selectedObjectIDs.count
    }

    @discardableResult func lineEndpoint(at point:SIMD2<Float>,tolerance:Float=14)->(id:UUID,endpoint:Int)?{
        let c=canvasPoint(from:point)
        guard let hit=objectStore.nearestLineEndpoint(to:CGPoint(x:CGFloat(c.x),y:CGFloat(c.y)),tolerance:CGFloat(tolerance/zoomScale)) else{return nil}
        return (hit.id,hit.endpoint)
    }

    @discardableResult func moveSelectedLineEndpoint(id:UUID,endpoint:Int,to point:SIMD2<Float>)->Bool{
        let c=canvasPoint(from:point)
        guard objectStore.moveLineEndpoint(id:id,endpoint:endpoint,to:CGPoint(x:CGFloat(c.x),y:CGFloat(c.y))) else{return false}
        rebuildGeometry();return true
    }

    func deleteSelectedObjects(){guard !selectedObjectIDs.isEmpty else{return};recordMutation();for id in selectedObjectIDs{objectStore.remove(id:id)};selectedObjectIDs=[];customRotationCenter=nil;rebuildGeometry()}

    @discardableResult func eraseObjectsByScribble(_ path:[SIMD2<Float>],tolerance:Float=12)->Bool{
        guard path.count>=2 else{return false}
        let cpath=path.map{let p=canvasPoint(from:$0);return CGPoint(x:CGFloat(p.x),y:CGFloat(p.y))}
        let removed=objectStore.eraseByScribble(cpath,tolerance:CGFloat(tolerance/zoomScale))
        if removed.isEmpty{return false}
        selectedObjectIDs.removeAll{removed.contains($0)}
        rebuildGeometry();return true
    }

    @discardableResult func selectStroke(at point:SIMD2<Float>,tolerance:Float=10)->Bool{let c=canvasPoint(from:point),tol=tolerance/zoomScale;var best:Int?;var bd=tol;for i in committedStrokes.indices.reversed(){let s=committedStrokes[i].points;guard s.count>=2 else{continue};for n in 0..<(s.count-1){let d=distanceFromPoint(c,toSegment:SIMD2(s[n].x,s[n].y),SIMD2(s[n+1].x,s[n+1].y));if d<=bd{bd=d;best=i;break}}};selectedStrokeIndices=best.map{[$0]} ?? [];selectedObjectIDs=[];customRotationCenter=nil;rebuildGeometry();return best != nil}
    @discardableResult func toggleStroke(at point:SIMD2<Float>,tolerance:Float=10)->Bool{let c=canvasPoint(from:point),tol=tolerance/zoomScale;var hit:Int?;for i in committedStrokes.indices.reversed(){let s=committedStrokes[i].points;guard s.count>=2 else{continue};for n in 0..<(s.count-1) where distanceFromPoint(c,toSegment:SIMD2(s[n].x,s[n].y),SIMD2(s[n+1].x,s[n+1].y))<=tol{hit=i;break};if hit != nil{break}};guard let i=hit else{return false};if selectedStrokeIndices.contains(i){selectedStrokeIndices.removeAll{$0==i}}else{selectedStrokeIndices.append(i)};selectedObjectIDs=[];rebuildGeometry();return true}
    @discardableResult func selectStrokes(in viewRect:CGRect,fullyContained:Bool=false)->Int{let p0=canvasPoint(from:SIMD2(Float(viewRect.minX),Float(viewRect.minY))),p1=canvasPoint(from:SIMD2(Float(viewRect.maxX),Float(viewRect.maxY)));let r=CGRect(x:CGFloat(min(p0.x,p1.x)),y:CGFloat(min(p0.y,p1.y)),width:CGFloat(abs(p1.x-p0.x)),height:CGFloat(abs(p1.y-p0.y)));let result=committedStrokes.indices.filter{guard let b=strokeBounds(committedStrokes[$0].points) else{return false};return fullyContained ? r.contains(b):r.intersects(b)};selectedStrokeIndices=Array(result);selectedObjectIDs=[];customRotationCenter=nil;rebuildGeometry();return result.count}
    ...
    func selectionBounds()->CGRect?{
        var result:CGRect?
        for i in selectedStrokeIndices where committedStrokes.indices.contains(i){if let b=strokeBounds(committedStrokes[i].points){result=result?.union(b) ?? b}}
        for id in selectedObjectIDs{if let b=objectStore.bounds(of:id){result=result?.union(b) ?? b}}
        guard let r=result else{return nil};return r.insetBy(dx:-8,dy:-8)
    }
    ...
    func resizeSelected(handle:SelectionHandle,to point:SIMD2<Float>){guard !selectedStrokeIndices.isEmpty,selectedObjectIDs.isEmpty,let r=selectionBounds() else{return};let p=canvasPoint(from:point);let a:SIMD2<Float>;switch handle{case .topLeft:a=SIMD2(Float(r.maxX),Float(r.maxY));case .topRight:a=SIMD2(Float(r.minX),Float(r.maxY));case .bottomLeft:a=SIMD2(Float(r.maxX),Float(r.minY));case .bottomRight:a=SIMD2(Float(r.minX),Float(r.minY))};let ow=max(Float(r.width),1),oh=max(Float(r.height),1),sx=max(abs(p.x-a.x),1)/ow,sy=max(abs(p.y-a.y),1)/oh;for i in selectedStrokeIndices where committedStrokes.indices.contains(i){committedStrokes[i].points=committedStrokes[i].points.map{InkPoint(x:a.x+($0.x-a.x)*sx,y:a.y+($0.y-a.y)*sy,pressure:$0.pressure)}};rebuildGeometry()}
    func scaleSelected(by f:Float){guard f>0,let c=selectionCenter() else{return};if !selectedObjectIDs.isEmpty{for id in selectedObjectIDs{guard let object=objectStore.object(with:id) else{continue};let current=object.transform.scale;_ = objectStore.transform(id:id,scale:CGSize(width:current.width*CGFloat(f),height:current.height*CGFloat(f)))}}else{for i in selectedStrokeIndices where committedStrokes.indices.contains(i){committedStrokes[i].points=committedStrokes[i].points.map{InkPoint(x:c.x+($0.x-c.x)*f,y:c.y+($0.y-c.y)*f,pressure:$0.pressure)}}};rebuildGeometry()}
    func moveSelected(by d:SIMD2<Float>){if !selectedObjectIDs.isEmpty{for id in selectedObjectIDs{guard let object=objectStore.object(with:id) else{continue};let p=object.transform.position;_ = objectStore.transform(id:id,position:CGPoint(x:p.x+CGFloat(d.x),y:p.y+CGFloat(d.y)))}}else{for i in selectedStrokeIndices where committedStrokes.indices.contains(i){committedStrokes[i].points=committedStrokes[i].points.map{InkPoint(x:$0.x+d.x,y:$0.y+d.y,pressure:$0.pressure)}}};rebuildGeometry()}
    ...
    func setSelectedRotationDegrees(_ degrees:Double){guard selectedObjectIDs.isEmpty,let c=selectionCenter() else{return};rotateSelected(by:Float(degrees-selectedRotationDegrees)*.pi/180,center:c)}
    private func rotateSelected(by d:Float,center c:SIMD2<Float>){if !selectedObjectIDs.isEmpty{for id in selectedObjectIDs{guard let object=objectStore.object(with:id) else{continue};_ = objectStore.transform(id:id,rotation:object.transform.rotation+CGFloat(d))}}else{let co=cos(d),si=sin(d);for i in selectedStrokeIndices where committedStrokes.indices.contains(i){committedStrokes[i].points=committedStrokes[i].points.map{let v=SIMD2($0.x,$0.y)-c;return InkPoint(x:v.x*co-v.y*si+c.x,y:v.x*si+v.y*co+c.y,pressure:$0.pressure)};committedStrokes[i].rotation += d}};rebuildGeometry()}
    func reflectSelected(horizontal:Bool){guard let c=selectionCenter() else{return};if !selectedObjectIDs.isEmpty{for id in selectedObjectIDs{guard let object=objectStore.object(with:id) else{continue};let s=object.transform.scale;_ = objectStore.transform(id:id,scale:CGSize(width:horizontal ? -s.width:s.width,height:horizontal ? s.height:-s.height))}}else{for i in selectedStrokeIndices where committedStrokes.indices.contains(i){committedStrokes[i].points=committedStrokes[i].points.map{horizontal ? InkPoint(x:2*c.x-$0.x,y:$0.y,pressure:$0.pressure):InkPoint(x:$0.x,y:2*c.y-$0.y,pressure:$0.pressure)}}};rebuildGeometry()}
    func deleteSelected(){if !selectedObjectIDs.isEmpty{deleteSelectedObjects();return};guard !selectedStrokeIndices.isEmpty else{return};recordMutation();for i in selectedStrokeIndices.sorted(by: >) where committedStrokes.indices.contains(i){committedStrokes.remove(at:i)};selectedStrokeIndices=[];customRotationCenter=nil;rebuildGeometry()}
    ...
    private func rebuildGeometry(){var o:[InkVertex]=[];appendBackgroundPattern(to:&o);for i in committedStrokes.indices{let s=committedStrokes[i];appendStrokeGeometry(s.points,style:s.style,to:&o)};appendStructuredObjects(to:&o);if let r=selectionBounds(){appendSelectionBounds(r,to:&o)};if !activeStroke.isEmpty{appendStrokeGeometry(activeStroke,style:penStyle,to:&o)};vertices=o;vertexBuffer=vertices.isEmpty ? nil:device.makeBuffer(bytes:vertices,length:vertices.count*MemoryLayout<InkVertex>.stride,options:.storageModeShared)}
    private func appendStructuredObjects(to o:inout[InkVertex]){for object in objectStore.objects{switch object.kind{case .line,.arrow:guard object.geometry.points.count>=2 else{continue};let transformed=objectStore.transformedPoints(of:object);guard transformed.count>=2 else{continue};let a=transformed[0],b=transformed[1];let color=metalColor(object.style);let p0=viewPosition(SIMD2(Float(a.x),Float(a.y))),p1=viewPosition(SIMD2(Float(b.x),Float(b.y)));appendLineQuad(p0,p1,width:Float(max(0.5,object.style.strokeWidth)),color:color,to:&o);default:break}}}
