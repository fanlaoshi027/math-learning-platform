...
final class InkRenderer: NSObject, MTKViewDelegate {
    ...
    private struct HistoryState { var strokes: [StoredStroke]; var objects: [GraphicObject]; var selection: [Int] }
    ...
    /// Commits a straight line as a structured GraphicObject. It is deliberately
    /// separate from freehand strokes so the line remains editable after saving.
    ...
    func deleteSelected(){guard !selectedStrokeIndices.isEmpty else{return};recordMutation();for i in selectedStrokeIndices.sorted(by: >) where committedStrokes.indices.contains(i){committedStrokes.remove(at:i)};selectedStrokeIndices=[];customRotationCenter=nil;rebuildGeometry()}
