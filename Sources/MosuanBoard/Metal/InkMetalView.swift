...
final class InkMetalView: MTKView {
    ...
    override func mouseDown(with event:NSEvent){window?.makeFirstResponder(self);let p=makePoint(from:event)
        if event.modifierFlags.contains(.command) && isSelectionTool { return }
        if event.keyCodeSafeSpace || NSEvent.modifierFlagsContainSpace(event) { panDrag=true;lastPoint=p;return }
        if isEraserTool {renderer.beginHistoryTransaction();eraserPoints=[p];return}
        if isSelectionTool {renderer.beginHistoryTransaction();if renderer.rotationHandle(at:p){rotationDrag=true;lastRotationPoint=p;return};if let h=renderer.selectionHandle(at:p){resizeHandle=h;lastPoint=p;return};selectionDrag=renderer.selectStroke(at:p);lastPoint=p;onSelectionChanged?();draw();return}
    ...
    override func keyUp(with event:NSEvent){super.keyUp(with:event)}
...
private extension NSEvent {
    var keyCodeSafeSpace: Bool { keyCode == 49 }
    static func modifierFlagsContainSpace(_ event:NSEvent)->Bool { event.keyCode == 49 }
}
