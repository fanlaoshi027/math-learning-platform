...
final class InkMetalView: MTKView {
    ...
    private func makePoint(from event:NSEvent)->SIMD2<Float>{let p=convert(event.locationInWindow,from:nil);return renderer.canvasPoint(from:SIMD2(Float(p.x),Float(p.y)))}
}
