...
class Site_Settings {
    ...
    public function sanitize($input) { $input=is_array($input)?$input:array(); $out=array(); foreach(self::defaults() as $key=>$default) { $value=isset($input[$key])?$input[$key]:$default; if('wechat_image'===$key){$out[$key]=esc_url_raw($value);} elseif('home_default_tab'===$key){$out[$key]=in_array($value,array('topic','supplementary'),true)?$value:$default;} else {$out[$key]=sanitize_textarea_field($value);} } return $out; }
